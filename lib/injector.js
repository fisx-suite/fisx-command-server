...
function getRequireConfig(options, context) {
    ...
    if (!customConfig || (!customConfig.baseUrl && requireConfig.baseUrl === 'src')) {
...
function injector(options, callback) {
    return function (context) {
        ...
        var unzipDoneHandler = function (err, buffer) {
            if (err) {
                fis.log.warn(err);
            }
            else {
        ...
        switch (contentEncoding) {
            case 'gzip':
                zlib.unzip(context.content, unzipDoneHandler);
                break;
            case 'deflate':
                zlib.deflate(context.content, unzipDoneHandler);
                break;
            default:
                inject();
        }
...
function htmlUpdate(options, context, content) {
    var replacer = options.replacer;
    if (typeof replacer === 'function') {
        content = replacer(content, context);
    }
    else if (_.isPlainObject(replacer)) {
        for (var k in replacer) {
            if (replacer.hasOwnProperty(k)) {
                content = content.toString().replace(
                    new RegExp(util.escapeRegExp(k), 'ig'),
                    replacer[k]
                );
            }
        }
...
exports.getRequireConfigInjector = function (serverOption, options) {
    ...
    return injector(options, function (context, content) {
        ...
        htmlUpdate(options, context, context.content);
    });
};
 ...
 * @return {Function}
 */
exports.getHtmlUpdater = function (serverOption, options) {
    options || (options = {});
    _.assign(options, serverOption);

    return injector(options, function (context, content) {
        htmlUpdate(options, context, content);
    });
};
