...
function getRequireStyleFiles(context, reqPathName, content) {
    ...
    while ((result = requireRegexp.exec(content))) {
        ...
        if (styleFileRegExp.test(id)) {
            ...
            var pathName = '/'
                + path.join(
                    path.dirname(reqPathName),
                    styleUrl.pathname
                );
            var absPath = path.join(docRoot, pathName);
            if (!util.isFileExists(absPath)) {
                if (!/^\./.test(id)) {
                    var depDir = fis.get('component.installDir');
                    pathName = '/' + path.join(depDir, id);
                }
            }
            styleFiles.push({
                id: id,
...
function processCssRequire(context, isInline, styleFileMap) {
    ...
    content = content.replace(
        ...
        function (match, prefix, quot, id) {
            ...
            // 内联样式文件：重新计算样式文件引用的 url 资源路径
            if (isInline && fis.util.normalizeCSSURLResource) {
                insertStyle = fis.util.normalizeCSSURLResource(
                    styleFile.path, insertStyle,
                    // 这里 `index.html` 随便指定的一个文件，只要是计算路径
                    // 为相对项目根目录下的绝对路径即可
...
function requireCssProcess(context, options) {
    var reqPathName = context.request.pathname.substr(1);
    var styleFiles = getRequireStyleFiles(
        context, reqPathName, context.content.toString()
    );
