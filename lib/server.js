...
function injectCustomProcessor(config, startOpts) {
    ...
    injectResource && injectResource({
        ...
        requireConfigInjector: injector.getRequireConfigInjector.bind(this, startOpts),
        htmlUpdater: injector.getHtmlUpdater.bind(this, startOpts),
        babelProcessor: require('./babel-processor'),
        vueProcessor: require('./vue-processor'),
        reactProcessor: require('./react-processor'),
        composeProcessor: require('./compose-processor'),
        envifyProcessor: require('./envify-processor'),
        requireCssProcessor: require('./require-css-processor')
    });
}

function addWatchReloadSupport(options) {
...
function injectLocationHandlers(config) {
    var getLocations = config.getLocations;

    config.getLocations = function () {
        var result = getLocations.apply(this, arguments);
        Array.prototype.unshift.apply(result, [
            require('./insert-css-handler')
        ]);
...
function initServerConfig(options) {
    ...
    // 注入全局资源处理接口
    injectCustomProcessor(config, options);

    // 添加 watchrealod 支持
    addWatchReloadSupport(options);

    // 注入请求处理器
    injectLocationHandlers(config);
...
exports.start = function (options) {
    var config = initServerConfig(options);
    if (!config) {
        return;
    }
