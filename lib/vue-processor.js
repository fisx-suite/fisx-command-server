...
function processVueFile(filePath, context, vueLoader) {
    var content = util.readFile(filePath);
    var timer = util.timer();
    timer.start();
    var result = vueLoader.compile(filePath, content, {
        script: {
            lang: 'babel'
        },
        sourceMap: false,
...
function getVueHandlers(options) {
    ...
    return {
        ...
        hotReloadApi: {
            location: /\/src\/hot\-reload\-api\.js($|\?)/,
            handler: [
                function (context) {
                    if (!hotReloadApi) {
